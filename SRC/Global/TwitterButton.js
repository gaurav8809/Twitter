...
export const SystemButton = props => {
  let {styles, onPress, opacity, text, aOpacity} = props;
  ...
  return (
    <View style={[Styles.sysView, styles && styles.view, {opacity: opacity}]}>
      <TouchableOpacity
        ...
        onPress={onPress}
        disabled={dis}>
        <Text style={[Styles.sysText, styles && styles.text]}>{text}</Text>
      </TouchableOpacity>
    </View>
  );
};
...
export const BlueWhiteButton = props => {
  let {onPress, text, activeText, btnStatus, useColor, btnStyle, activeOpacity = 1} = props;
...
export const LinerButton = props => {
  let {onPress, text, useColor, btnStyle, activeOpacity, textStyle} = props;
...
export const BubbleButton = props => {
  let {onPress, IconDetails, screen} = props;
  let component;
