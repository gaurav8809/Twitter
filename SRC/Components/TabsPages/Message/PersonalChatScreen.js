...
const PersonalChatScreen = props => {
  ...
  let {id, profileImage, profilename, username} = chatDataParam.userInfo;
  ...
  }, [CurrentChat.channelID, channelID, chatDataParam.userInfo, dispatch]);

  useEffect(() => {
    if (channelID !== null) {
      let dataObj = ME.id;
      props
        .changeStatus(channelID, dataObj, typeStatus)
        .then(response => {})
        .catch(error => {
          console.log(error);
        });
    }
  }, [ME.id, channelID, props, typeStatus]);
  ...
  const DMYFormat = date => {
    ...
    if (TODAY) {
      return `${getFor(hour > 12 ? hour % 12 : hour)}:${getFor(
        minute,
      )} ${AMPM}`;
    } else if (YESTERDAY) {
      return `Yesterday, ${getFor(hour > 12 ? hour % 12 : hour)}:${getFor(
        minute,
      )} ${AMPM}`;
    } else {
      return (
        `${getFor(fdate)}/${getFor(month)}/${year}` +
  ...
  const renderMyChatBox = item => {
    let {messageText, timestamp, chatImage} = item;
    return (
      <View style={Styles.myBoxOuter}>
        ...
        <Text style={Styles.myDate}>{DMYFormat(timestamp)}</Text>
      </View>
    );
  };
          ...
          <View>
            ...
            {messageText !== '' && (
              <View
                ...
                <Text style={Styles.opMessageText}>{messageText}</Text>
              </View>
            )}
          </View>
        </View>
        <Text style={Styles.opDate}>{DMYFormat(timestamp)}</Text>
      </View>
    );
  };
  ...
  const sendMessageAction = () => {
    ...
    const callSendMessage = () => {
      if (channelID !== null) {
        ...
        props
          .sendMessage(channelID, dataObj, members)
          .then(response => {
            // let dataObj = {
            //     userInfo: item,
            //     chatInfo: [],
    ...
    if (selectedPhoto !== null) {
      ...
      props
        .FireBaseStoreData('ChatResources', selPhoto)
      ...
      <KeyboardAvoidingView
        ...
        <View style={{flex: 1, width: SW(0.94), alignSelf: 'center'}}>
          <FlatList
            ...
            ListHeaderComponent={<View style={{marginTop: SH(0.02)}} />}
            ListFooterComponent={<View style={{marginTop: SH(0.02)}} />}
