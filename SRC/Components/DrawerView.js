...
class DrawerView extends Component {
      ...
      .then(response => {
        if (response !== null) {
          ...
          const resetAction = StackActions.reset({
            index: 0,
            actions: [NavigationActions.navigate({routeName: 'HelloScreen'})],
          });
  ...
  renderMenuItem = (item, index) => {
    let {lisouterview, listtext} = Styles;
    return (
      <TouchableOpacity
        onPress={() => {
          item.text === 'Text Recognition' && this.props.navigation.navigate('TextRecognition');
          item.text === 'Image Labeling' && this.props.navigation.navigate('ImageRecognition');
        }}
        style={lisouterview}
        key={index}>
        <View>
          <Icon
            name={item.IconDetails.Name}
            color={'slategray'}
            type={item.IconDetails.Type}
            size={swidth * 0.07}
          />
        </View>
        <View style={{marginLeft: swidth * 0.04}}>
          <Text style={listtext}>{item.text}</Text>
        </View>
      </TouchableOpacity>
    );
  };
  ...
  LogOut = async () => {
    if (HELPER.AsyncRemove('AsyncLogedInUserData')) {
      const resetAction = StackActions.reset({
        index: 0,
        actions: [NavigationActions.navigate({routeName: 'StartUpLoader'})],
      });
      ...
      const DBRef = firestore()
        .collection('users')
        .doc(this.state.LogedInUserData.id);
  ...
  render() {
    ...
    let {LogedInUserData} = this.state;
            ...
            <View style={{width: swidth * 0.7}}>
              ...
              <TouchableOpacity
                ...
                <Text style={profilenametext}>
                  {LogedInUserData && `${LogedInUserData.profilename} `}
                  {LogedInUserData.official && <OfficialSymbol />}
                </Text>
                <Icon
              ...
              <TouchableOpacity
                ...
                <Text style={usernametext}>
                  {LogedInUserData && LogedInUserData.username}
                </Text>
              </TouchableOpacity>
              <View style={{marginTop: swidth * 0.04, flexDirection: 'row'}}>
                <TouchableOpacity
                  ...
                  <Text style={fValueText}>
                    ...
                    <Text style={fText}>{'Following   '}</Text>
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() =>
                  ...
                  <Text style={fValueText}>
                    ...
                    <Text style={fText}>{'Followers'}</Text>
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
          ...
          <View style={[Styles.settinglistview]}>
            <View style={{width: swidth * 0.7}}>
              ...
              {SETTINGLIST.map((item, index) =>
                this.renderSettingMenuItem(item, index),
              )}
            </View>
          </View>
        </View>
...
const mapStateToProps = state => {
  const {UserInfo, LogedInUserData} = state.UserReducer;
